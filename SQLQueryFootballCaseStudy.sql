...
-- Int is for Numbers with no decimal places
-- Date is for dates
...
FROM 'C:\Users\yerpu\OneDrive\Desktop\Data_Analytics\Football Case study\result 1872-2020.csv'
WITH ( FORMAT='csv');
...
FROM '"C:\Users\yerpu\OneDrive\Desktop\Data_Analytics\Football Case study\countries of the world.csv'
WITH ( FORMAT='CSV');
...
select * from dbo.Football_Data_2020
select * from dbo.Countries_Data
--show the total home score and gdp per capita for home team

select a.Home_team, home_score_total,b.GDP_$_per_capita from 
(select Home_Team, SUM(home_score) as home_score_total from dbo.Football_Data_2020 where Tournament = 'FIFA World Cup' group by Home_Team) a
left join
(select Country, GDP_$_per_capita from dbo.Countries_Data)b
on a.Home_team = b.Country
order by b.GDP_$_per_capita desc


-- Exm 2: LEFT JOIN  - Question: Show the total Home_Score, GDP Per capita for Home_Team and Region

select total_home_score,a.Home_Team, b.GDP_$_per_capita,b.Region from
(select SUM(home_score)as total_home_score,home_team  from dbo.Football_Data_2020 group by Home_Team) a 
LEFT JOIN
(select Country, Region, GDP_$_per_capita from dbo.Countries_Data) b 
on a.home_team = b.Country

-- Exm 3: LEFT JOIN - Total Home Score per Region


select sum(total_home_score) as total_home_score ,b.Region from
(select home_team, SUM(home_score) as total_home_score from dbo.Football_Data_2020 group by Home_Team) a
LEFT JOIN
(select country, region from dbo.Countries_Data) b
ON a.home_team = b.country
